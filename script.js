...
function drawGantt() {
  ...
  // ─ Barre marge libre ─
    if (showFree && task.ml !== undefined && task.ml !== null) {
      ...
      let barLeft, barWidth, barTooltipText;
      
      if (task.ml === 0) {
        // Marge libre nulle : barre de la même longueur que la tâche (position début)
        barLeft = task.start * scale;
        barWidth = Math.max(task.duration * scale, minBarPx);
        barTooltipText = `<strong>${task.name}</strong> — marge libre nulle<br>
          Aucune marge disponible<br>
          Durée : <b>${task.duration}</b>`;
        bar.style.opacity = "0.4";
      } else {
        // Marge libre positive : barre de même largeur que la tâche,
        // positionnée pour se terminer à la fin tardive (lateEnd)
        // Donc elle commence à lateStart = lateEnd - duration
        barLeft = task.lateStart * scale;
        barWidth = Math.max(task.duration * scale, minBarPx);
        barTooltipText = `<strong>${task.name}</strong> — marge libre<br>
          Début marge : <b>${task.lateStart}</b> &nbsp;·&nbsp; Fin marge : <b>${task.lateEnd}</b><br>
          Marge disponible : <b>${task.ml}</b><br>`;
      }
      ...
      bar.style.cssText = `
        ...
        top: ${showLate ? 24 : 6}px;
      `;
      
      bar.addEventListener("mouseenter", (e) =>
        showTooltip(e, barTooltipText)
      );
      bar.addEventListener("mousemove", moveTooltip);
      bar.addEventListener("mouseleave", hideTooltip);
      barsArea.appendChild(bar);
...
// ------------------------------------------------------------------
//  Mise à jour automatique après modification des données
// ------------------------------------------------------------------
function autoCalculateAndRefresh() {
  if (tasks.length === 0) {
    // Si aucune tâche, on vide l'affichage
  ...
  // Recalculer toutes les dates et marges
  calculate();
  // Les fonctions displayResult() et drawGantt() sont déjà appelées dans calculate()
  // Mais on s'assure que le carrousel est à jour
  updateTaskCarousel();
}
